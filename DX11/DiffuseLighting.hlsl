...
float4 PSMain(VSOUT input) : SV_TARGET
{
    ...
    float4 Ambient = { 0.02f, 0.02f, 0.02f, 0.02f };
   ...
   float roughnessValue = RoughnessMap.Sample(samplerRoughness, input.tex).r;
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        float4 diffuse = DoDiffuse(light, lightVector, transformedNormal);

       // float4 Specular = calcSpecular(lightVector, transformedNormal, input.viewVector, light.lightColor, 32.0f);
